...
Only download CC Switch Next from **[ccswitch.io](https://ccswitch.io)**, **[GitHub Releases](https://github.com/farion1231/cc-switch/releases)**, or the project source repository. Any "CC Switch Next" site or client that asks for payment, top-ups, or login credentials is not official.
...
The CLI tools managed by CC Switch Next (Claude Code, Codex, Gemini CLI) require a Node.js environment.
...
2. Open the DMG, or extract the zip to get `CC Switch Next.app`
3. Drag it to the Applications folder

### Signed and Notarized

CC Switch Next for macOS is signed and notarized by Apple. You can install and open it directly — no extra steps needed.
...
After installation, launch CC Switch Next:

1. The app window displays correctly
2. A CC Switch Next icon appears in the system tray
3. The app switcher shows enabled managed apps, and you can switch to the target app panel

## Auto Update

CC Switch Next includes built-in auto-update functionality:
...
- Move `CC Switch Next.app` to Trash
