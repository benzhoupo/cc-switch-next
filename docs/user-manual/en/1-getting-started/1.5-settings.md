...
This section describes how to configure CC Switch Next according to your preferences.
...
CC Switch Next supports four languages:
...
When enabled, CC Switch Next automatically runs when the system starts.
...
Starting from v3.13.0, CC Switch Next adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger CC Switch Next via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If CC Switch Next runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, CC Switch Next automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that CC Switch Next uses when opening a terminal.
...
The storage location for CC Switch Next's own data, defaulting to `~/.cc-switch/`.
...
Configure CC Switch Next's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, CC Switch Next automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
- CC Switch Next automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, CC Switch Next displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current CC Switch Next version number, with support for:
