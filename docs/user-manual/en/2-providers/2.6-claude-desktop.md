...
The Claude Desktop panel lets you manage Claude Desktop provider configurations in CC Switch Next.
...
Claude Desktop and Claude Code are separate app entry points. Claude Code uses `~/.claude/settings.json`, while Claude Desktop uses its own 3P profile configuration. In CC Switch Next they appear as separate apps: "Claude" and "Claude Desktop"; the icon badge in the bottom-right corner helps distinguish them.
...
| Third-party mode | Writes the 3P profile managed by CC Switch Next |
| MCP / Skills | Claude Desktop 3P profiles do not use CC Switch Next MCP / Skills sync |
...
- Providers that need routing: keep CC Switch Next running, turn on Claude Desktop local routing, then restart Claude Desktop
...
In direct mode, CC Switch Next points the Claude Desktop 3P profile to the provider endpoint:
...
- CC Switch Next local routing does not need to stay running while in use
...
Model mapping mode is for providers whose models are not `claude-sonnet-*` / `claude-opus-*` / `claude-haiku-*` role IDs (including legacy Claude IDs and non-Claude models such as DeepSeek and Kimi), or when CC Switch Next needs to convert the API format.

After **Needs model mapping** is enabled, Claude Desktop connects to the CC Switch Next local gateway:

```text
http://127.0.0.1:15721/claude-desktop
```

CC Switch Next handles:
...
4. Using the provider credentials saved in CC Switch Next to call upstream
...
The reason is that Claude Desktop now rejects models outside the Sonnet / Opus / Haiku role families, so CC Switch Next local routing performs a model mapping pass.
...
Model mapping mode needs CC Switch Next local routing to convert requests. Local routing is powerful but more complex, so the main-page routing toggle is hidden by default to avoid accidental clicks by users who do not need routing. When you need routing, show it manually.
...
CC Switch Next restores Claude Desktop's official 1P mode and removes the 3P profile managed by CC Switch Next.

Official mode does not need an API key or local routing.

When importing from Claude Code, CC Switch Next automatically adds **Claude Desktop Official**.

## Configuration File Locations

CC Switch Next writes Claude Desktop 3P configuration files.
...
These files are maintained automatically by CC Switch Next. Manual edits are not recommended. If configuration becomes inconsistent, enabling the current provider again usually repairs it.
...
| Local routing token has not been generated | Switch to the provider again; CC Switch Next will write a new local token |
...
- CC Switch Next is still running
...
- Model mapping mode: CC Switch Next must keep running, and Claude Desktop local routing must stay on

### Are real upstream model names written into Claude Desktop?

Not in model mapping mode. The Claude Desktop profile only stores safe Sonnet / Opus / Haiku role routes and display names. Real upstream model names stay in the CC Switch Next provider configuration and are mapped when requests pass through the local gateway.
