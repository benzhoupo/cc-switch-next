...
CC Switch Next comes pre-configured with the following GitHub repositories:
...
CC Switch Next provides powerful search and filter features:
...
CC Switch Next compares installed skills with the remote repository version using **SHA-256 content hashes**. Whenever the remote has any content changes, the corresponding local skill card automatically shows an "Update available" indicator.
...
2. CC Switch Next batch-downloads all skills with pending updates
...
| ------------------------ | --------------------------------------------------------------------- |
| **CC Switch Next built-in**   | Default location `~/.cc-switch/skills/`, managed by CC Switch Next         |
| **`~/.agents/skills`**   | A shared directory conforming to community agent tool conventions, better for cross-tool collaboration |

### How to Switch

Select the target storage location from the settings or management menu in the Skills panel. The switch **does not lose skill state** — CC Switch Next smoothly migrates existing skills to the new location.
...
v3.13.0 integrates **skills.sh** public registry search so you can discover community skills directly inside CC Switch Next.
...
- CC Switch Next has not finished the latest scan
