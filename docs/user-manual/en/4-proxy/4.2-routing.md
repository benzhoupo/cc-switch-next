...
App routing means letting CC Switch Next route a specific application's API requests through the local routing service.
...
When routing is enabled, CC Switch Next modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, CC Switch Next will:
...
Before enabling routing, CC Switch Next backs up the original configuration and restores it when disabled.
