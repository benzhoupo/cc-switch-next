...
Hermes uses YAML configuration. CC Switch Next writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to CC Switch Next's "providers") |
...
CC Switch Next's priority when modifying configurations:

1. **CC Switch Next Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by CC Switch Next)
- CC Switch Next's `settings.json`
...
CC Switch Next v3.7.0 migrated from JSON files to SQLite:
