...
CC Switch Next for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/farion1231/cc-switch/releases).
...
2. Add CC Switch Next to your antivirus software's whitelist
...
**Solutions**:
1. Confirm the file was exported by CC Switch Next
...
**Windows**:
- Check taskbar settings to ensure the CC Switch Next icon is not hidden
...
Toggle "Lightweight Mode" from the system tray menu. The main window closes, and CC Switch Next runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, CC Switch Next covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `ccswitch://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
