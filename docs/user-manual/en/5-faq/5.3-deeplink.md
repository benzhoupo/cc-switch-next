...
CC Switch Next supports the `ccswitch://` deep link protocol, enabling one-click configuration import via links.
...
CC Switch Next provides an online deep link generator tool:
...
Using CC Switch Next's official online deep link generator tool is more convenient.
...
1. The system asks whether to open CC Switch Next
2. After confirming, CC Switch Next opens
...
CC Switch Next automatically registers the `ccswitch://` protocol during installation.
...
```bash
/usr/bin/open -a "CC Switch Next" --args --register-protocol
```
...
Before import, CC Switch Next will:
