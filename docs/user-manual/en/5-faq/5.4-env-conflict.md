...
CC Switch Next automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with CC Switch Next configuration
[Expand] [Dismiss]
```
...
3. CC Switch Next will automatically back up and remove the selected variables
...
If you prefer not to use CC Switch Next to remove variables, you can handle them manually:
...
- CC Switch Next provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use CC Switch Next to manage configurations**: Avoid setting API keys in system environment variables
