...
请只从 **[ccswitch.io](https://ccswitch.io)**、**[GitHub Releases](https://github.com/farion1231/cc-switch/releases)** 或项目源码仓库获取 CC Switch Next。任何要求付费、充值或索取登录凭据的“CC Switch Next”网站或客户端都不是官方渠道。
...
CC Switch Next 管理的 CLI 工具（Claude Code、Codex、Gemini CLI）需要 Node.js 环境。
...
2. 打开 DMG，或解压 zip 得到 `CC Switch Next.app`
3. 拖动到「应用程序」文件夹

### 已签名并公证

CC Switch Next macOS 版本已通过 Apple 代码签名和公证，可直接安装打开，无需额外操作。
...
安装完成后，启动 CC Switch Next：

1. 应用窗口正常显示
2. 系统托盘出现 CC Switch Next 图标
3. 应用切换器中能看到已启用的受管应用，并能切换到目标应用面板

## 自动更新

CC Switch Next 内置自动更新功能：
