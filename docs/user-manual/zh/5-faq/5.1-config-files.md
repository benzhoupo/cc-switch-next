...
Hermes 使用 YAML 配置。CC Switch Next 将 MCP 服务器写入 `mcp_servers`，将可编辑的供应商条目写入 `custom_providers`，读取 Hermes `providers` 字典中的只读条目，并在切换供应商时更新 `model.provider` / `model.default`。
...
| 字段 | 说明 |
|------|------|
| `models.providers` | 供应商配置（映射为 CC Switch Next 的"供应商"） |
| `env` | 环境变量配置 |
...
CC Switch Next 修改配置时的优先级：

1. **CC Switch Next 数据库** - 单一事实源 (SSOT)
...
- CLI 工具的配置文件（会被 CC Switch Next 回填）
- CC Switch Next 的 `settings.json`
...
CC Switch Next v3.7.0 从 JSON 文件迁移到 SQLite：
