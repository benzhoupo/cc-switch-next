...
pub fn run() {
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 CC Switch Next 的工作方式。
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older CC Switch Next version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            2) If you see “database version is newer”, please upgrade CC Switch Next\n\
