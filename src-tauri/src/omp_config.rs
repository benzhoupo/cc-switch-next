...
pub fn get_providers() -> Result<serde_json::Map<String, serde_json::Value>, AppError> {
    ...
    if let Some(providers) = config.get("providers").and_then(|v| v.as_mapping()) {
        for (key, value) in providers {
            if let Some(id) = key.as_str() {
                ...
                let json_val: serde_json::Value = serde_yaml::from_str(&yaml_str).map_err(|e| {
...
pub fn set_provider(name: &str, provider_config: serde_json::Value) -> Result<(), AppError> {
    ...
    if let Some(providers) = config.get_mut("providers").and_then(|v| v.as_mapping_mut()) {
...
pub fn set_mcp_server(id: &str, config: serde_json::Value) -> Result<(), AppError> {
    ...
    if full_config.get("mcpServers").is_none() {
        full_config["mcpServers"] = serde_yaml::Value::Mapping(serde_yaml::Mapping::new());
    }
